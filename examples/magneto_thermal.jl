...
q = add_post_quantity_term!(pp_darwin, "a")
add!(q, "Term", "{a}"; In="Domain_Mag", Jacobian="Vol")

q = add_post_quantity_term!(pp_darwin, "az")
add!(q, "Term", "CompZ[{a}]"; In="Domain_Mag", Jacobian="Vol")

q = add_post_quantity_term!(pp_darwin, "b")
add!(q, "Term", "{d a}"; In="Domain_Mag", Jacobian="Vol")

q = add_post_quantity_term!(pp_darwin, "bm")
add!(q, "Term", "Norm[{d a}]"; In="Domain_Mag", Jacobian="Vol")

q = add_post_quantity_term!(pp_darwin, "j")
...
q = add_post_quantity_term!(pp_darwin, "jz")
...
q = add_post_quantity_term!(pp_darwin, "jm")
...
q = add_post_quantity_term!(pp_darwin, "JouleLosses"; comment=" global losses")
...
add_operation!(op1, "Print[ jz, OnElementsOf Region[{DomainC_Mag}], Smoothing 1, Name \"jz [A/m²] Conducting domain\", File StrCat[ \"$(joinpath(output_dir,"jz_"))\", Sprintf(\"%g\",active_con), \".pos\" ] ];")
add_operation!(op1, "Print[ rhoj2, OnElementsOf Region[{DomainC_Mag}], Smoothing 1, Name \"Power density\", File StrCat[ \"$(joinpath(output_dir,"rhoj2_"))\", Sprintf(\"%g\",active_con), \".pos\" ] ];")
add_operation!(op1, "Print[ jm, OnElementsOf DomainC_Mag, Smoothing 1, Name \"|j| [A/m²] Conducting domain\", File StrCat[ \"$(joinpath(output_dir,"jm_"))\", Sprintf(\"%g\",active_con), \".pos\" ] ];")
add_operation!(op1, "Print[ dm, OnElementsOf DomainC_Mag, Smoothing 1, Name \"|D| [A/m²]\", File StrCat[ \"$(joinpath(output_dir,"dm_"))\", Sprintf(\"%g\",active_con), \".pos\" ] ];",
)

po2 = add!(postoperation, "Mag_Global", "Darwin_a_2D")
op2 = add_operation!(po2)
add_operation!(op2, "Print[ R, OnRegion Ind_1, Format Table, File \"R.dat\" ];")
add_operation!(op2, "Print[ L, OnRegion Ind_1, Format Table, File \"L.dat\" ];")

# Add thermal post-operations
po_the = add!(postoperation, "The_Maps", "ThermalSta")
op_the = add_operation!(po_the)

# All the thermal domain but the cable
add_operation!(op_the, "Print[ TinC, OnElementsOf Region[{Vol_Thermal,-Cable}], Smoothing 1, Name \"T [°C] araound cable\",  File \"T.pos\" ];")
add_operation!(op_the, "Print[ TinC , OnElementsOf Cable,  Name \"T [°C] Cable\", File \"T_cable.pos\" ];")
add_operation!(op_the, "Print[ q , OnElementsOf Region[{Vol_Thermal,-Cable}], Name \"heat flux [W/m²] around cable\",  File \"q.pos\" ];")
add_operation!(op_the, "Print[ q , OnElementsOf Cable, Name \"heat flux [W/m²] cable\",  File \"q_cable.pos\" ];")
